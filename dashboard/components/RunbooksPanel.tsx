...
const RISK_STYLES: Record<string, string> = {
  low: "bg-pilot-success/10 text-pilot-success border-pilot-success/30",
  medium: "bg-pilot-warning/10 text-pilot-warning border-pilot-warning/30",
  high: "bg-pilot-danger/10 text-pilot-danger border-pilot-danger/30",
};
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-[380px_1fr] gap-6">
      ...
      <aside className="space-y-2">
        <h2 className="text-base font-bold font-display text-pilot-text-primary flex items-center gap-2 mb-3">
            ...
            >
              <div className="flex items-start justify-between gap-2 mb-1">
                <div className="flex items-center gap-2 min-w-0">
                  ...
                  <span className="font-semibold text-sm text-pilot-text-primary truncate">{rb.name}</span>
                </div>
          ...
          <div className="bg-pilot-surface border border-pilot-border rounded-xl p-6 space-y-5">
            <div>
              <div className="flex items-center gap-3 mb-2">
                ...
                <h3 className="text-xl font-bold font-display text-pilot-text-primary">{selected.name}</h3>
                <span
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 pt-2 border-t border-pilot-border">
              <div>
                ...
                <input
                  ...
                  placeholder="e.g. dev"
                  className="w-full bg-pilot-bg border border-pilot-border rounded-md px-3 py-1.5 text-sm text-pilot-text-primary placeholder:text-pilot-muted focus:outline-none focus:border-pilot-accent/60 focus:ring-2 focus:ring-pilot-accent/25"
                />
              </div>
              <div>
                <label className="eyebrow block mb-1">
                    ...
                    <div className="flex items-start justify-between gap-3">
                      <div className="min-w-0 flex-1">
                        <div className="flex items-center gap-2 mb-1">
                          ...
                          {isManual && (
                            <span className="text-2xs text-pilot-warning bg-pilot-warning/10 px-1.5 py-0.5 rounded">
                              manual
                            </span>
                          )}
                          {!isManual && (
                            <span className="text-2xs text-pilot-accent bg-pilot-accent/10 px-1.5 py-0.5 rounded font-mono">
                              {action}
                            </span>
                          )}
                        </div>
                        <p className="text-sm text-pilot-text-primary leading-relaxed">{step}</p>
                      </div>
                      <button
                        onClick={() => handleExecute(selected, i)}
                        disabled={isExecuting}
                        className="shrink-0 flex items-center gap-1.5 text-xs font-semibold px-3 py-1.5 rounded-md bg-pilot-accent hover:bg-pilot-accent-light text-pilot-bg disabled:opacity-50"
                      >
                        {isExecuting ? (
                    ...
                    {result && (
                      <div
                        className={`mt-3 flex items-start gap-2 text-xs p-2 rounded-md font-mono leading-relaxed ${
                          result.status === "ok"
                            ? "bg-pilot-success/10 text-pilot-success"
                            : result.status === "error"
                            ? "bg-pilot-danger/10 text-pilot-danger"
                            : "bg-pilot-warning/10 text-pilot-warning"
                        }`}
                      >

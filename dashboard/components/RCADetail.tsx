...
const SEVERITY_COLORS: Record<Severity, string> = {
  ...
  high: "text-pilot-danger bg-pilot-danger/10",
  medium: "text-pilot-warning bg-pilot-warning/12",
  low: "text-pilot-info bg-pilot-info/12",
  info: "text-pilot-muted bg-pilot-surface-2",
};
    ...
    <div className="space-y-4">
      ...
      <div className="flex items-start justify-between">
        <div>
          ...
          <h2 className="text-lg font-bold font-display text-pilot-text-primary">
      ...
      {/* Root Cause */}
      <section className="bg-pilot-surface border border-pilot-border rounded-xl p-4 shadow-card">
        <h3 className="text-sm font-bold font-display text-pilot-text-primary mb-2">Root Cause</h3>
        ...
        <p className="text-sm text-pilot-text-primary">{report.root_cause.detail}</p>
      ...
      <section className="bg-pilot-surface border border-pilot-border rounded-xl p-4 shadow-card">
        <button
          ...
          className="flex items-center gap-2 text-sm font-bold font-display text-pilot-text-primary w-full text-left"
        >
        ...
        {expandedEvidence && report.evidence_chain && (
          <div className="mt-3 space-y-2">
            {report.evidence_chain.map((evidence, i) => (
              <div key={i} className="bg-pilot-bg rounded-lg p-3 text-xs">
                <div className="flex items-center gap-2 mb-1">
          ...
          <div className="space-y-2">
            {report.remediation.map((step, i) => {
              ...
              return (
                <div
                  key={i}
                  className="flex items-start justify-between bg-pilot-bg rounded-lg p-3 gap-3"
                >
                  <div className="flex-1">
                    ...
                    {step.command && (
                      <code className="text-xs font-mono text-pilot-muted mt-1 block bg-pilot-surface-2 px-2 py-1 rounded">
                        {step.command}
                      </code>
                    )}
                    {result && (
                  ...
                  <button
                    ...
                    className="shrink-0 flex items-center gap-1 text-xs px-3 py-1.5 rounded-lg font-semibold bg-pilot-accent hover:bg-pilot-accent-light text-pilot-bg disabled:opacity-50"
                  >
